 ...
 * 이메일도 함께 옮겨야 한다.
 *
 * `zerostore.kr` 은 실재하는 .kr 도메인이고, 이 프로젝트가 소유했다는 보장이 없다
 * (등록 여부를 확인하지 않고 정했다). 지금은 이 이메일로 실제 메일을 보내는 흐름이
 * 앱에 없어서(비밀번호 재설정 없음, 계정은 관리자가 대시보드에서 직접 만듦) 남의
 * 도메인이어도 당장 문제가 없다. **비밀번호 재설정처럼 메일을 발송하는 기능을 켜는
 * 순간, 전 계정의 메일이 이 도메인의 실제 소유자에게 전달될 수 있다.** 그런 기능을
 * 넣기 전에 반드시 도메인 소유를 확인하거나, 소유가 확인된 도메인으로 바꿔라.
 */
