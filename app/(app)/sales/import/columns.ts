...
const GUESS: [ColumnKey, string[]][] = [
  ...
  // 판매가와 맞아야 한다"였는데, 매칭은 바코드·상품명·pos_name 으로만 하고
  // 그 값을 등록 판매가와 대조하는 곳은 어디에도 없다. 그래서 남는 효과는
  // **받지도 않은 돈을 매출로 적는 것**뿐이었다: 오픈특가 반값행사 90개가
  // 148,500원이 아니라 297,000원으로 들어가고, 이 매장 8월 4일 파일 하나에서만
  // 매출이 277,200원(9.7%) 부풀었다. 사용자가 실매출로 결정했다(2026-08-05).
  //
  // '순매출' 은 여전히 넣지 마라 — 부가세를 뺀 값이라 반대로 10% 작게 박힌다.
  // '객단가' 도 넣지 마라 — 이 시트에서는 우연히 실매출÷수량과 같지만 표준
  // 뜻은 매출÷거래건수(손님 1인당)라 수량이 여러 개인 줄에서 조용히 틀린다.
  //
  // '매출' 은 뒤에 둔다 — 이 매장 시트에는 '매출' 이라는 열이 없고,
  // 앞에 두면 '실매출'·'매출금액' 보다 먼저 집으려 들어 뜻이 흐려진다.
  ['amount', ['실매출', '매출금액', '금액', '합계', '판매금액', '매출액', '합계금액', 'amount', 'total', '총액', '매출']],
