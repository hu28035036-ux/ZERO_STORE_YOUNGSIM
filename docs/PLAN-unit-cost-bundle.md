...
2026-08-01 사용자와 대화로 확정한 설계다. 구현 전에 `docs/HANDOFF.md` 를 먼저
읽어라. DB 는 프로덕션이고 **실사용 상품 358개가 이미 들어와 있다** (문서를
처음 쓸 때는 비어 있다고 알았으나 같은 날 확인으로 뒤집혔다 — 아래 2차 결정
절).

> **구현됐다 (2026-08-01, 마이그레이션 0017).** 단, 스키마 한 곳이 이 문서와
> 다르다: `products.purchase_unit_qty` 를 만들지 않았다. 이 설계가 쓰인 뒤
> 같은 날 0016(상품 임포트)이 **`variants.units_per_pack`(입수)** 을 먼저
> 만들어 실데이터 354건을 채웠기 때문이다 — 같은 뜻의 컬럼을 products 에 또
> 만들면 입수의 진실이 두 곳이 되고, 수정 화면에 "입수" 입력이 두 개 생긴다.
> 그래서 **입수는 units_per_pack 하나**로 가고, 상품에는 `unit`(세는 말)과
> `purchase_unit_name`(묶음 이름)만 더했다. 박스 토글·환산 도우미·입고 박스
> 모드는 전부 변형별 units_per_pack 을 읽는다. 짝 제약(chk_purchase_unit_pair)
> 은 그래서 없다 — 이름은 라벨일 뿐이고 비면 화면이 '박스'로 부른다.
> 아래 본문은 결정 당시 기록 그대로 두었다. 스키마·RPC 의 정본은
> `supabase/migrations/20260801000002_unit_and_purchase_unit.sql` 이다.
...
- ~~출고의 박스 단위~~ — **구현 후 사용자 요청으로 추가됐다 (2026-08-01).**
  박스째 폐기·이동이 실제로 있었다. 입고와 같은 토글이고, 입수가 없는 상품은
  입출고 화면에서 바로 입수를 받아 상품에 저장한다 (매번 묻지 않기 위해).
- **실사·판매의 박스 단위.** 가게는 낱개로 팔고 낱개로 센다.
