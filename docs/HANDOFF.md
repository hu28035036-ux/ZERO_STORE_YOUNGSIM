...
아홉 경로(재고 목록에 상품 수정 화면이 더해졌다), 프로덕션 빌드 기준이다. 그
과정에서 **로그인이 통째로 막혀 있던 것**을 찾아 고쳤고(계정 데이터 문제였다),
판매 화면에서 금액이 잘리는 레이아웃 버그도 고쳤다.

이 세션 안에서 그 뒤로 두 가지가 더 있었다: **상품 수정 화면을 새로 만들었고**
(`/stock/[productId]/edit`, "만든 것" 표 참고), **로그인 방식을 이메일에서
아이디로 바꿨다**(`e63843e`) — 계정도 아이디 체계에 맞게 교체했다(아래 "DB 현재
상태" 참고). **Vercel 배포도 끝났다** (https://zero-store-youngsim.vercel.app,
`README.md` 의 "배포" 절 참고).

아직 안 한 것: 실제 바코드 스캐너 하드웨어 검증. 자세한 건 "검증 상태" 절을 봐라.
...
`/stock/new`(변형별 바코드 칸)와 `/movements/new`(찾기 칸)에는 카메라로 바코드를
찍어 넣는 버튼이 이번에 붙었다 — 화면 구조는 그대로고 입력 수단 하나가 늘었을
뿐이다. 위 표의 `/sell` 바코드 스캔은 이것과 별개다 — 값을 키보드처럼 흘려
넣는 물리 스캐너용 입력칸이고, 카메라는 아직 안 붙었다(붙일 수는 있다 — 아래
"이번에 생긴 공용 조각" 참고).
...
| `components/scanner/barcode-scanner.tsx` | 카메라 오버레이 `<BarcodeScanner open onDetect onClose>`. 상품 등록·입출고 찾기가 쓴다. `/sell` 은 아직 안 붙였지만 같은 컴포넌트를 그대로 가져다 쓸 수 있다 |
...
### DB 는 비어 있다 — 데모 데이터를 2026-07-31 에 지웠다

사용자가 다 확인한 뒤 아래 SQL 로 정리했다. 이제 **실제로 쓰기 시작하는 상태**다.
